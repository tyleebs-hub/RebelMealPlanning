...
const HOUSEHOLD_DESC: Record<string, string> = {
  leber: "the Leber family (Tyler, Charity, and 2 young kids) in Southwest Washington",
  mom: "Mom's household in Southwest Washington (three adults: Mom, Dad, and their adult son — no young kids)",
};
const householdDesc = (ctx: PlanningContext) => HOUSEHOLD_DESC[ctx.household] ?? "the household";

export function buildSystem(ctx: PlanningContext): string {
  const { dinnerServings, lunchServings, targetDinners, targetLunches } = ctx.cfg;
  return `You are the meal-planning assistant for ${householdDesc(ctx)}. You propose weekly dinner and lunch plans. You do not do arithmetic on portions or decide whether a week is "covered" — you propose recipes, day placements, and batch multipliers, and the app's ledger verifies coverage. Return your answer ONLY by calling the provided tool.
...
- A dinner cook feeds the household that night (${dinnerServings} servings reserved). Leftover servings become packed lunches (${lunchServings} servings each = one lunch slot).
- Only recipes flagged "reheats well" should feed lunches. Component batches (is_component) are lunch building blocks.
- Weekly targets: ${targetDinners} dinners and ${targetLunches} lunches (=${targetLunches * lunchServings} lunch portions). Close the lunch gap by raising a dinner's multiplier or adding a dedicated prep/component batch — never by proposing a recipe that doesn't reheat.
...
export function buildChatSystem(ctx: PlanningContext): string {
  const cook = ctx.household === "leber" ? "Tyler" : "you";
  return `You are a warm, practical meal-planning helper for ${householdDesc(ctx)}. You brainstorm dinners and lunches for THIS week's open slots.

Be concise and concrete. When ${cook} mention${cook === "you" ? "" : "s"} ingredients (often on sale), suggest a few specific ways to use them across the open slots — point to existing recipes in the library by exact title when one fits, or propose a simple new dish that suits their tastes. For each idea, say which open day/slot it could fill and whether it reheats for packed lunches. Prefer batch-cook ideas that stretch into lunches when there's a lunch gap. Keep answers short — a few tight bullets, not essays.
...
- Weekly targets: ${ctx.cfg.targetDinners} dinners, ${ctx.cfg.targetLunches * ctx.cfg.lunchServings} lunch portions. Only recipes that reheat well should feed lunches.

When you propose specific dinners or lunches that could drop into an open slot, ALSO call the suggest_meals tool with them as structured actions — each with a day (an OPEN slot, never Friday dinner), a meal, and either the exact library recipe_id (if it's an existing recipe) or, for a NEW dish, a short ingredients list, a few steps, and whether it reheats_well. Keep your text reply conversational and readable; the tool is only for the "Add" buttons, so don't describe the tool. Only include meals you'd genuinely recommend — it's fine to call it with fewer meals than you discuss, or not at all for a general question.`;
}
