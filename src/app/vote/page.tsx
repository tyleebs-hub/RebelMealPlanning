            ...
            {sentIdeas.length > 0 && (
              <div className="mt-3">
                ...
                <ul className="mt-2 flex flex-col gap-1.5">
                  {sentIdeas.map((s) => (
                    <li key={s.suggestionId} className="flex items-center justify-between gap-3 rounded-lg border border-[var(--rule)] bg-[var(--card)] px-3 py-2 text-sm">
                      <span>
                        <span className="font-medium">{s.title}</span>
                        {s.note && <span className="ml-2 text-[var(--ink2)]">{s.note}</span>}
                      </span>
                      <form action={removeSuggestion.bind(null, start, s.suggestionId)}>
                        <button aria-label={`Remove ${s.title}`} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--ink2)] hover:bg-[var(--rule2)] hover:text-[var(--clay-bg)]">
                          ✕
                        </button>
                      </form>
                    </li>
